    ...
    public class DriverManager
    {
        public static async Task<bool> UpdateDrivers(string DriverRepo = null)
        {
            ...
            if (string.IsNullOrEmpty(DriverRepo))
            {
                ...
                {
                    StorageFile driverPackage = await ResourcesManager.RetrieveFile(ResourcesManager.DownloadableComponent.DRIVERS_MH2LM, true);
            ...
            if (DeviceManager.Device.Product == DeviceProduct.Mh2lm)
            {
                DriverDefinitions = $"{DriverRepo}\\definitions\\Desktop\\ARM64\\Internal\\mh2lm.xml";
            }
            else
            {
                throw new Exception("Unknown device product");
